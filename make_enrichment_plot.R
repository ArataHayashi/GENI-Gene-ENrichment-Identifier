...
make_enrichment_plot <- function (x, geneSetID, title = "", color = "green", base_size = 11, 
                       rel_heights = c(1.5, 0.5, 1), subplots = 1:3, pvalue_table = FALSE, 
                       ES_geom = "line", nrank, nhigh) {
  ...
  p.pos <- p + geom_segment(data = df2, aes_(x = ~x, xend = ~x, 
    ...
    annotate(geom = "text", x = nrank*0.20, y = nhigh, label = "pos. corr",color="red")+
    annotate(geom = "text", x = nrank*0.80, y = nhigh, label = "neg. corr",color="steelblue")+
